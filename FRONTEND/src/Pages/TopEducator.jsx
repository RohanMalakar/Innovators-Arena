...
const TopEducator = () => (
  <div className='bg-white text-white py-10'>
    <div className='text-center'>
      <h1 className='text-4xl text-red-500  font-bold '>CORE-MEMBERS</h1>
    ...
    <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-10'>
      {leadershipData.map(({ id, name, title, bio, imageUrl, twitterUrl, githubUrl, slackUrl }) => (
        <div key={id} className='relative max-w-sm mx-auto overflow-hidden  group bg-red-900 shadow-lg shadow-black rounded-lg  transition-transform duration-1000 transform hover:scale-90 hover:bg-black  cursor-pointer p-6 text-center '>
          ...
          <div className='flex justify-center mt-4 space-x-3'>
            <a href={twitterUrl}><FaTwitter className='text-red-300' /></a>
            <a href={githubUrl}><FaGithub className='text-red-300' /></a>
            <a href={slackUrl}><FaSlackHash className='text-red-300' /></a>
          </div>
          <div className="absolute inset-0 bg-gray-400 bg-opacity-30 transform -translate-y-full transition-transform duration-1000 ease-in-out group-hover:translate-y-0"></div>
        </div>
      ))}
    </div>
  </div>
